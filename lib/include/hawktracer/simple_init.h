...
#define HT_INIT_GLOBAL_LISTENERS_P(LISTENER_MACRO, ...) LISTENER_MACRO(ht_global_callstack_timeline_get(), __VA_ARGS__)
...
#define HT_SIMPLE_DEINIT(...) \
    do { \
        ...
        ht_timeline_unregister_all_listeners(HT_TIMELINE(ht_global_callstack_timeline_get())); \
