...
#define HT_TP_STRACEPOINT(timeline, label) \
    static HT_THREAD_LOCAL const char* HT_UNIQUE_VAR_NAME(fnc_track) = ht_feature_cached_string_add_mapping(timeline, label); \
...
#define HT_TP_STRACEPOINT(timeline, label) \
    ...
    static HT_THREAD_LOCAL const char* HT_UNIQUE_VAR_NAME(fnc_track) = label; \
