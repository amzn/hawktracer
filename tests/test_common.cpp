...
void mixed_test_listener(TEventPtr events, size_t event_count, HT_Boolean is_serialized, void* user_data)
{
    ...
    while (events < end)
    {
        if (HT_EVENT_GET_CLASS(events)->type == ht_HT_CallstackIntEvent_get_event_klass_instance()->type)
        {
            i->int_values.push_back(*((HT_CallstackIntEvent*)events));
        }
        else if (HT_EVENT_GET_CLASS(events)->type == ht_HT_CallstackStringEvent_get_event_klass_instance()->type)
        {
